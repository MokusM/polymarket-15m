...
# Contract price zone — перевіряється у scanner по CLOB ask (реальна ціна входу)
CONTRACT_PRICE_MIN = float(os.getenv("CONTRACT_PRICE_MIN", "0.50"))
CONTRACT_PRICE_MAX = float(os.getenv("CONTRACT_PRICE_MAX", "0.90"))
...
GAP_MIN_USD = float(os.getenv("GAP_MIN_USD", "30"))

# OBI filter: bid_volume / ask_volume must exceed this ratio (1.0 = neutral)
OBI_MIN_RATIO = float(os.getenv("OBI_MIN_RATIO", "1.05"))
